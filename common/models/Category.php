...
//            [['parent_id'], 'exist', 'skipOnError' => true, 'targetClass' => Category::className(), 'targetAttribute' => ['parent_id' => 'id']],
    ...
    {
        $top = self::find()->where('parent_id = :pid', [":pid" => 0])->limit(11)->orderby('created_at asc')->asArray()->all();
        $data = [];
        foreach((array)$top as $k=>$cate) {
            $cate['children'] = self::find()->where("parent_id = :pid", [":pid" => $cate['id']])->limit(10)->asArray()->all();
            foreach ((array)$cate['children'] as $j=>$cate3){
                $cate3['grandson'] = self::find()->where("parent_id = :pid", [":pid" => $cate3['id']])->limit(10)->asArray()->all();
                $cate['children'][$j] = $cate3;
            }
            $data[$k] = $cate;
        }
        return $data;
